...
#  variables (compose will provide them; nothing here is
#  wired into docker yet):
#
...
#                        rides on it)
#    DATABASE_URL      — sqlite:///... today; the scheme
#                        picks the engine, so a later move
#                        to postgres://... is an env change
#
...
############################################################
#
# Aware UTC everywhere (USE_TZ); the API publishes ISO-8601
# strings with explicit offsets — knfapp/common/timestamps.py
# is the one place that knows the stamp policy, and it is the
# shape the production data already carries, so existing
# clients and rows keep parsing.
############################################################
...
############################################################
#
# SQLite in a volume today, postgres later by changing
# DATABASE_URL alone. ATOMIC_REQUESTS wraps every request in
# one transaction: register's invitation-code burn, user
# INSERT and session mint commit together or not at all.
############################################################
