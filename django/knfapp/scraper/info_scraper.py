...
#  The info views overlay the blobs on the curated fallback
#  at request time.
#
#  No section can fail the run: each extractor runs in its
#  own guard, so a dead contacts page costs the contacts
#  section and nothing else, and the run is closed as
#  'completed' with the failed section named in
#  error_message.
#
...
#  this is public.
#
#  Neighbours: the cron command runs scrape_faculty_info
#  every 24 h and api/views.py exposes it to admins as POST
#  /api/scraper/info. Every run lands in scraper_runs under
#  source 'knf.vu.lt/info': articles_found holds contacts +
#  programs found, and articles_new the number of SECTIONS
#  whose blob actually changed (the column names are the
#  news scraper's; /api/scraper/status also serves them
#  under the neutral itemsFound/itemsNew keys).
############################################################
...
# Wall-clock budget for one run — five pages, generous — and
# open_run's staleness horizon for this source
RUN_BUDGET_SECONDS = 300

# One info run at a time in this process; open_run guards
# the other processes
...
# <div> wrapping several entry <p>s would produce one
# glued-together item AND every inner <p>'s item. Only LEAF
# candidates are read — an element containing another
...
# page does not state it: a fixed "4 metai" / "2 metai"
# would publish factually wrong course lengths for every
...
# vedėja") — the role the page actually states, never a
# reconstruction that copies the name back into the
...
# in the document would hide the real "knf@knf.vu.lt"
# further down and drop the email field altogether.
#
...
# actually changed, so the status page says something true
# for this source. Returns {pages_scraped, contacts_found,
# programs_found}, plus "error" on failure and
# {"skipped": True} when another run holds the source.
#
...
#   - api/views.py — trigger_info_scrape, the admin POST
#     /api/scraper/info
############################################################

def scrape_faculty_info() -> dict:
    # STEP 1: one info run at a time, in this process and any
    # other
    # =======================================================
    ...
    try:
        run_id = open_run("knf.vu.lt/info", RUN_BUDGET_SECONDS)
        if run_id is None:
            return {"pages_scraped": 0, "contacts_found": 0, "programs_found": 0, "skipped": True}

        deadline = run_deadline(RUN_BUDGET_SECONDS)
        return _run(run_id, deadline)
    finally:
        _RUN_LOCK.release()


def _run(run_id, deadline):
    try:
        ...
        # general_contact obeys that rule too: its fields are
        ...
        if general:
            scraped_data_lt["general_contact"] = {
                **GENERAL_CONTACT_DEFAULTS,
                **_stored_section("lt", "general_contact"),
                **general,
            }

        changed_sections = _store_info("lt", scraped_data_lt, now) if scraped_data_lt else 0
        ...
        # =======================================================
        close_run(run_id, contacts_found + programs_found, changed_sections,
                  error_message="; ".join(section_errors) or None)
        ...
        # The atomic block in _store_info has already rolled its
        # pending section writes back; the run row is closed
        # through a freshened connection
